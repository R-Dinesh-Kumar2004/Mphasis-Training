package com.hibernate.inh.single;

import java.util.List;

import org.hibernate.Session;

import com.hibernate.utils.HBUtils;

public class SingleTableGetDemo {

	public static void main(String[] args) {
		
		Session session = HBUtils.getSessionFactory().openSession();
		
		//get 1 contract employee
		ContractEmployee contractEmployee = session.get(ContractEmployee.class, 3);
		System.out.println(contractEmployee);
		
		//get list of contract employee
		List<ContractEmployee> emp = session.createQuery("from ContractEmployee",ContractEmployee.class).getResultList();
		emp.forEach(System.out::println);
		
		session.close();
	}

}
