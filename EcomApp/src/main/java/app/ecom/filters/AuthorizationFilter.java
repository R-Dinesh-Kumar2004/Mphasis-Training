...
public class AuthorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res,
        ...
        if (uri.contains("/admin/") && "ADMIN".equals(role)) {
        	System.out.println("admin role before dofilter");
            chain.doFilter(req, res);
            System.out.println("admin role after dofilter");
            return;
        }

        if (uri.contains("/subadmin/") && "SUB_ADMIN".equals(role)) {
        	System.out.println("sub admin role before dofilter");
            chain.doFilter(req, res);
            System.out.println("sub admin role after dofilter");
            return;
        }
